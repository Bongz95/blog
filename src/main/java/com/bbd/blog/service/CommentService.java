...
public class CommentService {
	...
	public static boolean comment(Comment comment, Reader reader) {
		String sql = String.format("INSERT INTO COMMENT(user_id, comment_text, comment_datetime, post_id) "
				+ "OUTPUT INSERTED.comment_id,INSERTED.comment_datetime VALUES (%d, '%s','%s', CURRENT_TIMESTAMP, %d);",reader.getId(), comment.getText(), comment.getPost().getId(), comment.getDatetime());
	...
	public static List<Comment> getComments(Reader reader){
		...
		try {
			while(rs.next()) {
				...
				Comment c = new Comment(id, text, d, p);
				reader.addComment(c);
				list.add(c);
			}
		} catch (SQLException e) {
